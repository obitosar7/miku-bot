const handler = async (m, { conn, command, text }) => {
  ...
  const response =
    `*♥️╎تـم إرســال الـقـلــوب بـنـجــاح˙⁠❥⁠*`    

  async function loading() {
var hawemod = [
"*♥️╎یـتـم إرســال الـقـلــوب ˙⁠❥⁠*",
"❤️",
"🧡",
"💛",
"💚",
...
"*✅╎انـتـهــت˙⁠❥⁠*"
   ...
   let { key } = await conn.sendMessage(m.chat, {text: `❀° ┄───────╭\n\n*⏳╎إنـتـظــر لـحـظــه مــن فـضـلـك . . .˙⁠❥*\n\n╯───────┄ °❀`, mentions: conn.parseMention(response)}, {quoted: m})
...
handler.help = ['love'];
handler.tags = ['fun'];
handler.command = /^(قلب|قلوب|القلب|القلوب)$/i;
